	...
	return (
		<main className="min-h-dvh bg-cover bg-center px-4 py-10 text-[#f5df9a] sm:px-8" style={{ backgroundImage: `url(${fundo})` }}>
			<form onSubmit={submit} className="mx-auto max-w-5xl rounded-[1.5rem] border-2 border-[#b08a3c] bg-[#060d29]/95 p-7 shadow-2xl sm:p-12" style={{ borderImage: `url(${borda}) 28 round` }}>
				...
				<img src={divisor} alt="" className="mx-auto my-8 h-7 w-full object-contain" />
				<div className="grid gap-x-8 gap-y-6 sm:grid-cols-2 lg:grid-cols-3">
					{fields.map(([key, label, type = 'text']) => (
						<label key={key} className="block"><span className="mb-2 block font-serif text-sm uppercase">{label}</span><input type={type} step={key === 'preco' ? '0.01' : undefined} value={livro[key] ?? ''} onChange={(event) => update(key, event.target.value)} className="w-full border-b-2 border-[#b59750] bg-transparent px-3 py-2 font-serif text-[#fff1bd] outline-none" style={{ backgroundImage: `url(${texto})`, backgroundSize: '100% 100%' }} /></label>
					))}
				</div>
